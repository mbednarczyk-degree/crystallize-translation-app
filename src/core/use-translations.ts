        ...
        chunkComponents.forEach((chunkComponent, chunkComponentIndex) => {
          (async () => {
            ...
            try {
              ...
              const data = await translateComponentType(translateLanguage, chunkComponent, preferences, { translator });
  ...
  const handleNameTranslation = useCallback(
    async ({ property, propertyIndex, preferences }: HandlePropertyProps) => {
      ...
      try {
        ...
        preferences.shouldPushTranslationToDraft && !!updatedProperty && onUpdateComponent(updatedProperty, "property");
      } catch {
        updateProperty({
          propertyIndex,
          translationState: "error",
