...
function componentHandler(component: Component) {
    switch (component.type) {
        ...
        case ComponentType.ContentChunk: {
            if (
                !component.content ||
                !(component.content as ContentChunkContent).chunks?.length
            ) {
                return null;
            }

            const transformedChunks = (
                component.content as ContentChunkContent
            ).chunks.map((chunk) => chunk.map(toComponentInput));
        ...
        case ComponentType.Files: {
            ...
            return (component.content as FileContent).files?.map(
                ({ key, title }) => ({ key, title })
            );
        }
        case ComponentType.GridRelations: {
            const ids =
                (component?.content as GridRelationsContent)?.grids?.map(
                    (item) => item.id
                ) || [];
        ...
        case ComponentType.Images: {
            ...
            return (component.content as ImageContent).images
                ?.filter((image) => !!image.key)
                .map(handleImage);
        }
        case ComponentType.ItemRelations: {
            const ids =
                (component?.content as ItemRelationsContent)?.items?.map(
                    (item) => item.id
                ) || [];
            const skus =
                (component?.content as ItemRelationsContent)?.productVariants?.map(
                    (item) => item.sku
                ) || [];
                        ...
                        return {
                            ...
                            images:
                                images && images?.length
                                    ? images?.filter((image) => !!image.key).map(handleImage)
                                    : null,
                            videos:
        ...
        case ComponentType.Selection: {
            if (
                !component.content ||
                !(component.content as SelectionContent).options
            ) {
                return null;
            }

            return {
                keys: (component.content as SelectionContent).options?.map(
                    ({ key }) => key
                ),
            };
        }
        ...
        case ComponentType.Videos: {
            ...
            return (component.content as VideoContent)?.videos
                ?.map(handleVideo)
                .filter((v) => v.key);
        }
